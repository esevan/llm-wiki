...
- SQLite stores workflow, settings, indexes, jobs, notifications, and localization metadata. The
  Markdown vault adapter is the only vault filesystem boundary. Provider secrets use OS keyring.
...
| UI-013 | Completion Review and verify | Evidence review can fail/retry; only human verify completes; one notification published | job, vault playbook, archive, score | `test_completion_dashboard.py`, CB-006/013/017 | `features/completion` | high / high | PASS | PASS | PASS | PASS |
...
| UI-015 | Search vault | Empty/loading/result/pagination; lexical fallback when semantic unavailable | index reads; SSE refresh | `test_retrieval.py`, CB-001, PA-001 | `features/search` | medium / high | PASS | PASS | PASS | PASS |
