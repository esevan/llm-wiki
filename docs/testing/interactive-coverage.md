...
| F32 | Refinement proposal document preview; `RefinementPanel.tsx` | proposal readback renders beside the conversation | session loaded; the conversation and unapplied proposal document remain visible together in a bounded modal, including empty/generating states | P(part): packaged modal geometry at 1200px and 900px, visible composer, and separate preview/chat scrolling; long-field scrolling requires rendered visual review |
...
| F34 | Saved refinement note + message-scroll autosave; `RefinementPanel.tsx` | debounced workspace PUT | session loaded; collapsed note disclosure opens to edit, note and anchor resume after relaunch | P(part): disclosure, note, scroll, close/reopen; debounce failure and relaunch absent |
