...
아래 스크린샷은 이 안내를 위해 캡처한 workflow 예시를 보존합니다. 현재 앱은 다음 절에서 설명하는
차분한 크림색 작업공간과 옅은 레일 표현을 사용하므로, 화면 표면의 스타일은 이 과거 이미지와 다를 수
있습니다.

## 작업공간 읽기

LLM Wiki는 따뜻한 크림색 작업공간과 차분한 옅은 내비게이션 레일, 절제된 핑크색 강조를 사용합니다.
현재 화면은 주 작업을 먼저 보여주며, Capture는 작게 유지하고 진행 중인 Solution, workflow lane,
재사용할 맥락에는 가장 강한 시각적 비중을 둡니다. 카드, 서랍, 대화상자, Queue, 알림은 읽기 쉬운
표면·포커스·상태 표현을 함께 사용합니다. 검색은 로컬 Vault 안내로 시작하고 로딩·빈 결과·실패 상태를
그 자리에 보여줍니다. 좁은 데스크톱 폭에서도 내비게이션이 줄바꿈되고 Workbench 동작은 넘치지 않고
계속 접근할 수 있습니다. 이러한 표현 변경은 승인 경계를 바꾸지 않습니다. AI는 작업을 준비하거나
정리할 수 있지만, workflow 전환은 계속 사용자가 선택합니다.
