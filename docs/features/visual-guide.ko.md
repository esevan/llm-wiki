...
진행 중인 Task 카드는 처음 남긴 내용의 유무와 관계없이 같은 너비와 높이로 한 줄에 표시되며 좌우로 스크롤합니다. 긴 카드 내용은 하단에 정렬된 액션 버튼 위에서 스크롤됩니다. 나란히 표시되는 진행 중 패널과 최근 완료 목록은 같은 높이를 유지하며, 완료 목록은 독립적으로 스크롤됩니다.
