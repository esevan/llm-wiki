...
export function ChatTrackingSurface() {
  ...
  return <>
    {snapshot.actions?.length ? <nav aria-label={t('chat.actions')} data-tracking-busy={String(Boolean(snapshot.busy))} data-tracking-error={snapshot.error ?? ''}>{snapshot.actions.map(action => <button type="button" key={action} data-control="chat-propose" data-action={action} disabled={snapshot.busy} onClick={() => window.dispatchEvent(new CustomEvent('llm-wiki:chat-tracking-propose',{detail:{action}}))}>{t('action.'+action)}</button>)}</nav> : null}
    ...
    {editing && <form data-control="tracking-json-form" onSubmit={event => { event.preventDefault(); try { const payload: unknown = JSON.parse(text); if (!payload || typeof payload !== 'object' || Array.isArray(payload)) throw new Error(t('chat.invalidObject')); act('edit', editing, payload); setEditing(null); } catch (caught) { setError(String(caught)); } }}>
      <label>{t('chat.edit')}<textarea value={text} data-control="tracking-json-edit" onChange={event => setText(event.target.value)} required data-user-content /></label>
      {error && <p role="alert">{error}</p>}
      <button type="button" data-control="tracking-json-cancel" onClick={() => setEditing(null)}>{t('chat.cancel')}</button>
      <button type="submit" data-control="tracking-json-preview">{t('chat.preview')}</button>
    </form>}
  </>;
}
