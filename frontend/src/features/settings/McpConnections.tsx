...
export function McpConnections() {
  ...
  return <section className="mcp-connections" aria-labelledby="mcp-connections-title">
    ...
    <form onSubmit={save}><label>{t('connections.name')}<input name="name" required maxLength={80}/></label>
      <fieldset disabled={busy}><legend>{t('connections.grant')}</legend>{scopes.map(scope=><label key={scope}><input type="checkbox" name="scope" value={scope} defaultChecked={!['topic:read','workbench:overview:read','knowledge:publish'].includes(scope)}/>{t('scope.'+scope)}</label>)}</fieldset>
      <label>{t('connections.topics')}<input name="topics" maxLength={1200}/></label><button disabled={busy}>{t('connections.create')}</button>
    </form>
    ...
    <ul>{connections.map(connection=><li key={connection.id}><strong data-user-content>{connection.name}</strong> · {t('connectionState.'+connection.state)}
      <details><summary>{t('connections.access')}</summary><ul>{connection.scopes.map(scope=><li key={scope}>{t('scope.'+scope)}</li>)}</ul><p data-user-content>{connection.topicIds?.join(', ')}</p><code data-user-content>llm-wiki-desktop --mcp --connection {connection.id}</code></details>
      {connection.state==='active'&&<button type="button" disabled={busy} onClick={()=>void run(async()=>{const response=await window.llmWikiApplication.request({path:'/work-tracking/connections/'+encodeURIComponent(connection.id),method:'DELETE'});if(!response.ok)throw Error('request_failed');await load();})}>{t('connections.revoke')}</button>}
    </li>)}</ul>
    <details><summary>{t('topics.title')}</summary><p>{t('topics.description')}</p><form onSubmit={membership}>
      <label>{t('topics.id')}<input name="topic" required maxLength={120}/></label>
      <label>{t('topics.type')}<select name="type">{['vault','captures','problems','features'].map(type=><option key={type} value={type}>{t('memberType.'+type)}</option>)}</select></label>
      <label>{t('topics.member')}<input name="member" required maxLength={1000}/></label>
      <label>{t('topics.operation')}<select name="included"><option value="yes">{t('topics.include')}</option><option value="no">{t('topics.remove')}</option></select></label>
      <button disabled={busy}>{t('topics.save')}</button>
    </form></details>
  </section>;
}
