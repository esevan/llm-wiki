...
export function McpConnections() {
  ...
  return <section className="mcp-connections" aria-labelledby="mcp-connections-title">
    ...
    <form className="mcp-connection-form" data-control="mcp-connection-form" onSubmit={save}><label className="mcp-form-field">{t('connections.name')}<input data-control="mcp-connection-name" name="name" required maxLength={80}/></label>
      <fieldset className="mcp-scope-fieldset" disabled={busy}><legend>{t('connections.grant')}</legend><div className="mcp-scope-grid">{scopes.map(scope=><label className="mcp-scope-option" key={scope}><input type="checkbox" data-control={`mcp-scope-${scope}`} name="scope" value={scope} defaultChecked={!['topic:read','workbench:overview:read','knowledge:publish'].includes(scope)}/><span>{t('scope.'+scope)}</span></label>)}</div></fieldset>
      <div className="mcp-connection-actions"><label className="mcp-form-field">{t('connections.topics')}<input data-control="mcp-connection-topics" name="topics" maxLength={1200}/></label><button data-control="mcp-connection-create" disabled={busy}>{t('connections.create')}</button></div>
    </form>
    ...
    <details className="mcp-topic-details" data-control="mcp-topic-details"><summary>{t('topics.title')}</summary><p>{t('topics.description')}</p><form className="mcp-topic-form" data-control="mcp-topic-form" onSubmit={membership}>
      <label className="mcp-form-field">{t('topics.id')}<input data-control="mcp-topic-id" name="topic" required maxLength={120}/></label>
      <label className="mcp-form-field">{t('topics.type')}<select data-control="mcp-topic-type" name="type">{['vault','captures','problems','features'].map(type=><option key={type} value={type}>{t('memberType.'+type)}</option>)}</select></label>
      <label className="mcp-form-field">{t('topics.member')}<input data-control="mcp-topic-member" name="member" required maxLength={1000}/></label>
      <label className="mcp-form-field">{t('topics.operation')}<select data-control="mcp-topic-included" name="included"><option value="yes">{t('topics.include')}</option><option value="no">{t('topics.remove')}</option></select></label>
