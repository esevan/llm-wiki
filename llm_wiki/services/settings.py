...
class ProviderSettings:
    ...
    def __init__(self, db: sqlite3.Connection):
        self.db = db
        self.db.execute(
            "CREATE TABLE IF NOT EXISTS provider_settings (id INTEGER PRIMARY KEY CHECK(id=1), base_url TEXT NOT NULL, model TEXT NOT NULL DEFAULT '', advanced_model TEXT NOT NULL DEFAULT '', advanced_tasks TEXT NOT NULL DEFAULT '{}', report_language TEXT NOT NULL DEFAULT 'ko', async_worker_count INTEGER NOT NULL DEFAULT 2)"
        )
        ...
        self.db.execute(
            "INSERT OR IGNORE INTO provider_settings(id,base_url,model,advanced_model,advanced_tasks,report_language,async_worker_count) VALUES (1,?, '', '', '{}', 'ko', 2)",
            (DEFAULT_BASE_URL,),
        )
        self.db.commit()
    ...
    def public(self) -> dict[str, object]:
        ...
        try:
            ...
            saved_tasks = {}
        advanced_tasks = {
            task: bool(saved_tasks.get(task, default)) for task, default in ADVANCED_TASK_DEFAULTS.items()
        }
        return {
            "base_url": row[0],
            "model": row[1],
            "advanced_model": row[2],
            "advanced_tasks": advanced_tasks,
            "report_language": row[4] or "ko",
            "async_worker_count": int(row[5]),
    ...
    def save(
        ...
        api_key: str | None,
        advanced_model: str = "",
        advanced_tasks: dict[str, bool] | None = None,
        report_language: str | None = None,
        async_worker_count: int | None = None,
    ) -> None:
        normalized = {
            task: bool(selected) for task, selected in (advanced_tasks or {}).items() if task in ADVANCED_TASK_DEFAULTS
        }
        ...
        count = int(
            async_worker_count if async_worker_count is not None else self.public().get("async_worker_count", 2)
        )
        if not 1 <= count <= 32:
            raise ValueError("Async worker count must be between 1 and 32")
        self.db.execute(
            "UPDATE provider_settings SET base_url=?,model=?,advanced_model=?,advanced_tasks=?,report_language=?,async_worker_count=? WHERE id=1",
            (base_url.rstrip("/"), model.strip(), advanced_model.strip(), json.dumps(normalized), language, count),
        )
        self.db.commit()
        if api_key:
