...
class ProviderSettings:
    ...
    def __init__(self, db: sqlite3.Connection):
        self.db = db
        self.db.execute("CREATE TABLE IF NOT EXISTS provider_settings (id INTEGER PRIMARY KEY CHECK(id=1), base_url TEXT NOT NULL, model TEXT NOT NULL DEFAULT '', advanced_model TEXT NOT NULL DEFAULT '', advanced_tasks TEXT NOT NULL DEFAULT '{}', report_language TEXT NOT NULL DEFAULT 'ko')")
        ...
        self.db.execute("INSERT OR IGNORE INTO provider_settings(id,base_url,model,advanced_model,advanced_tasks,report_language) VALUES (1,?, '', '', '{}', 'ko')", (DEFAULT_BASE_URL,))
        self.db.commit()

    def public(self) -> dict[str, object]:
        row = self.db.execute("SELECT base_url,model,advanced_model,advanced_tasks,report_language FROM provider_settings WHERE id=1").fetchone()
        try:
        ...
        return {"base_url": row[0], "model": row[1], "advanced_model": row[2], "advanced_tasks": advanced_tasks, "report_language": row[4] or "ko", "api_key_configured": bool(self._secret())}

    def save(self, base_url: str, model: str, api_key: str | None, advanced_model: str = "", advanced_tasks: dict[str, bool] | None = None, report_language: str | None = None) -> None:
        ...
        self.db.execute("UPDATE provider_settings SET base_url=?,model=?,advanced_model=?,advanced_tasks=?,report_language=? WHERE id=1", (base_url.rstrip("/"), model.strip(), advanced_model.strip(), json.dumps(normalized), language))
        self.db.commit()
        if api_key:
