...
def build_lineage_document(db: Any, feature_id: str) -> tuple[dict[str, object], str]:
    ...
    capture_evidence = add_evidence("capture", capture.get("id", "missing"), "text", capture.get("text") or ABSENT_REASON, "captures" if capture else "")
    capture_claim = add_claim("stage:capture", "stage", "capture", capture.get("id", "missing"), "observed", capture.get("text") or ABSENT_REASON, [capture_evidence], material=True)

    desired_outcome = _section(str(problem.get("detail", "")), "desired outcome", "intended outcome", "목표", "의도한 결과")
    problem_text = str(problem.get("statement") or ABSENT_REASON)
    if desired_outcome:
        problem_text += f"\nDesired outcome: {desired_outcome}"
    problem_evidence = add_evidence("problem", problem.get("id", "missing"), "statement/detail", problem_text, "problems" if problem else "")
    problem_claim = add_claim("stage:problem", "stage", "problem", problem.get("id", "missing"), "observed", problem_text, [problem_evidence], material=True)
    ...
    solution_claim = add_claim("stage:solution", "stage", "solution", feature_id, "decided", solution_text, [solution_evidence], material=True)
    ...
    complete_claim = add_claim("stage:complete", "stage", "complete", complete_id, "decided", complete_text, [complete_evidence], material=True)
    ...
    promotion_claim = add_claim(
        "transition:capture-problem", "transition", "problem", problem.get("id", "missing"), "decided",
        promotion_text, [capture_evidence, problem_evidence], material=True,
    )
    ...
    solution_transition_claim = add_claim(
        "transition:problem-solution", "transition", "solution", feature_id, "decided",
        solution_change, [problem_evidence, solution_evidence, created_evidence], material=True,
    )
    completion_transition_claim = add_claim(
        "transition:solution-complete", "transition", "complete", complete_id, "decided",
        complete_text, [solution_evidence, complete_evidence], material=True,
    )
    ...
    for event in decisions:
        ...
        claim_key = add_claim(
            f"decision:{event['id']}", "decision_change", "solution", feature_id, "decided",
            change_text, [event_evidence], material=True,
        )
        decision_changes.append({"claim_key": claim_key, "event_type": event.get("event_type"), "created_at": event.get("created_at")})
    ...
    for report in conflict_reports:
        ...
        report_evidence = add_evidence("conflict_report", report["id"], "citation", report.get("citation") or ABSENT_REASON)
        evidence_keys = [report_evidence]
        if address:
            evidence_keys.append(add_evidence(
        ...
        claim_key = add_claim(
            f"conflict:{report['id']}", "conflict", "solution", feature_id,
            "decided" if status == "addressed" else "observed", text, evidence_keys, material=status != "unclear",
        )
        conflicts.append({
            "claim_key": claim_key,
            "report_id": report["id"],
    ...
    for item in checklist:
        ...
        key = add_claim(f"checklist:{item['id']}", "completion_evidence", "solution", feature_id, "observed", text, [ev])
    ...
    for review in reviews[-1:]:
        ...
        key = add_claim(f"review:{review['id']}", "completion_evidence", "complete", complete_id, "inferred", review_text, [ev], confidence="medium")
    ...
    document: dict[str, object] = {
        ...
        "lineage": {
            "stages": [
                {"kind": "capture", "record_type": "captures", "record_id": capture.get("id"), "title": "Capture", "claim_key": capture_claim, "occurred_at": capture.get("created_at"), "live_available": bool(capture)},
                {"kind": "problem", "record_type": "problems", "record_id": problem.get("id"), "title": "Problem", "claim_key": problem_claim, "occurred_at": problem.get("created_at"), "live_available": bool(problem)},
                {"kind": "solution", "record_type": "features", "record_id": feature_id, "title": "Solution", "claim_key": solution_claim, "occurred_at": feature.get("created_at"), "live_available": True},
                {"kind": "complete", "record_type": "", "record_id": complete_id, "title": "Complete", "claim_key": complete_claim, "occurred_at": latest_completion_decision.get("created_at") or completion.get("created_at"), "live_available": bool(completion or completion_decisions)},
            ],
            "transitions": [
                {"from": "capture", "to": "problem", "claim_key": promotion_claim, "context_kind": "recorded_change"},
                {"from": "problem", "to": "solution", "claim_key": solution_transition_claim, "context_kind": "decision_basis" if created_reason else "recorded_change", "material_conflict": material_conflict},
                {"from": "solution", "to": "complete", "claim_key": completion_transition_claim, "context_kind": "decision_basis" if complete_text != ABSENT_REASON else "recorded_change"},
            ],
        },
        "claims": claims,
...
def readable_report_context(lineages: list[dict[str, object]]) -> str:
    ...
    for lineage in lineages:
        ...
        if isinstance(evidence, list):
            evidence = {str(item.get("id") or item.get("key")): item for item in evidence if isinstance(item, dict)}
        claim_keys = {
            str(identifier): f"Claim {index}"
            for index, identifier in enumerate(claims, 1)
        }
        ...
        for claim in claims.values():
            for evidence_id in claim.get("evidence_ids", []):
                ...
                if not label:
                    ...
                    base = fixed_labels.get(source_type) or numbered_labels.get(source_type) or source_type.replace("_", " ").title()
        ...
        def safe(value: object) -> object:
            ...
            for key, item in value.items():
                if key == "claim_id":
                    ...
                    result["evidence_labels"] = [labels_by_evidence[str(identifier)] for identifier in item if str(identifier) in labels_by_evidence]
    ...
    for index, stage in enumerate(lineage.get("lineage", {}).get("stages", [])):
        ...
        if index < len(transitions):
            ...
            if transition.get("material_conflict"):
                ...
                lines.append(f"  - Conflict: {conflict.get('status')} · {conflict.get('disposition') or 'disposition not recorded'}")
