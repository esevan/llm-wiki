...
class CompletionReportHandler:
    ...
    async def __call__(self, context: HandlerContext) -> dict[str, Any]:
        ...
        lineages = (
            self.publisher.lineages(problem_id, refresh=True)
            if context.payload.get("refresh_lineage")
            else current_lineages
        )
        ...
        try:
            report = await provider.complete_json(
                [
                    {
                        "role": "system",
                        "content": 'Return JSON only: {"executive_summary_markdown":string,"report_body_markdown":string}. Prepare concise English-canonical Knowledge from supplied validated Lineage only. Preserve Observed, Decided, and Inferred distinctions; cite human-readable evidence labels; never expose internal IDs or invent facts.',
                    },
        ...
        if (
            not context.payload.get("refresh_lineage")
            and digest(json.dumps(self.publisher.lineages(problem_id), sort_keys=True, ensure_ascii=False))
            != context.source_hash
        ):
            raise StaleJobError("Completion evidence changed during report generation")
        self.publisher.workflow.record_ai_run(
            "problems",
            problem_id,
            "completion_executive_summary",
            "Validated Lineage",
            json.dumps(report, ensure_ascii=False),
        )
        return self.publisher.publish(
            problem_id, lineages=lineages, executive_summary=executive, report_body=body, status="generated"
        )
