...
from llm_wiki.services.jobs import RetryableJobError, StaleJobError
...
class AsyncJobWorker:
    def __init__(self, repository: JobRepository, registry: HandlerRegistry, *, lease_seconds: int = 30, worker_id: str | None = None):
    ...
    async def _execute(self, lease) -> None:
        ...
        try:
            ...
            while not task.done():
                ...
                if await self.repository.cancellation_requested(job.id):
                    ...
                    return
                if not await self.repository.heartbeat(lease, lease_seconds=self.lease_seconds):
                    task.cancel()
        ...
        except (RetryableJobError, httpx.TimeoutException, httpx.NetworkError) as error:
            ...
            if await self.repository.cancellation_requested(job.id):
                ...
                await self.repository.fail(job.id, lease.lease_token, "interrupted", "Job execution was interrupted", retryable=True)
        except StaleJobError as error:
            await self.repository.stale(job.id, lease.lease_token, str(error))
        except Exception as error:
            logger.exception("Durable job failed job_id=%s attempt=%s", job.id, lease.attempt)
