...
def organization_items(workflow: WorkflowEngine, locale: str) -> list[dict[str, object]]:
    ...
    return [{"entity_type": entity_type, "entity_id": item["id"], "title": item.get("statement") or item.get("title") or item.get("outcome") or item.get("text"), "state": item.get("state", "inbox"), "current_category": item.get("category", "")} for entity_type in ("captures", "problems", "features") for item in board[entity_type]]
    ...
    async def __call__(self, context: HandlerContext) -> dict[str, Any]:
        ...
        try:
            response = await provider.complete_json([
                {"role": "system", "content": "Return JSON only: {\"entries\":[{\"entity_type\":string,\"entity_id\":string,\"category\":string,\"attention_rank\":integer,\"rationale\":string}]}. Organize current work by attention without changing workflow states."},
                {"role": "system", "content": response_language_instruction(locale)},
                {"role": "user", "content": json.dumps({"items": items}, ensure_ascii=False)},
            ], "workbench organization")
        finally:
            await provider.aclose()
        if digest(json.dumps(organization_items(self.workflow, locale), sort_keys=True, ensure_ascii=False)) != context.source_hash:
