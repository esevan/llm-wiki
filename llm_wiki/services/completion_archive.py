...
class CompletionArchivePublisher:
    ...
    def __init__(self, workflow: WorkflowEngine, retrieval: RetrievalEngine, vault: MarkdownVaultAdapter, translations: VaultKnowledgeTranslationCache):
    ...
    def lineages(self, problem_id: str, *, refresh: bool = False) -> list[dict[str, Any]]:
        feature_ids = [str(row[0]) for row in self.workflow.db.execute("SELECT id FROM features WHERE problem_id=? ORDER BY created_at,rowid", (problem_id,))]
    ...
    def ensure_unmodified(self, problem_id: str):
        existing = self.retrieval.db.execute("SELECT path,source_hash FROM completion_playbooks WHERE problem_id=?", (problem_id,)).fetchone()
        if existing:
            try:
    ...
    def publish(self, problem_id: str, *, lineages: list[dict[str, Any]] | None = None, executive_summary: str = "", report_body: str = "", status: str = "deterministic_fallback") -> dict[str, Any]:
        ...
        path, content = self.workflow.completion_playbook(problem_id, directory, executive_summary=executive_summary, report_body=report_body, lineages=lineages)
        if existing:
        ...
        self.workflow.remember_completion_playbook(problem_id, path, digest(content), str(selected["snapshot_id"]) if selected else "", int(selected["version"]) if selected else 0, report_input_hash, status)
        self.retrieval.index_changed()
        return {"path": path, "raw_path": raw_path, "lineage": {"snapshot_id": selected["snapshot_id"], "status": selected["status"], "version": selected["version"], "retryable": bool(selected.get("generation", {}).get("inference_error"))} if selected else None, "report_generation": {"status": status, "lineage_snapshot_id": selected["snapshot_id"] if selected else "", "lineage_version": selected["version"] if selected else 0}}
